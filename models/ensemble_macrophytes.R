...
run_name <- mod_id
predictor_dir <- "../predictors/predictors_macrophytes"
obs_dir <- "../observations/macrophytes"
spec_list_file <- "spec_list_macrophytes.csv"
...
mod_methods <- c("RF", "XGBOOST", "GAM") # Subset
...
method_most <- method_tab$method[which.max(method_tab$num_presence)] # NOTE: for macrophytes we take max presences (as prevalence has been adjusted)
...
# Set custom settings
user.RF <- vector("list", n_rep)
names(user.RF) <- paste0("_allData_RUN", 1:n_rep)
user.RF <- lapply(user.RF, function(x) list(mtry = rf_mtry,
                                            ntree = rf_ntree,
