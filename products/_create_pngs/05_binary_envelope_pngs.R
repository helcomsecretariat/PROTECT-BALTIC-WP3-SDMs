...
for(j in 1:length(spec_group_all)){
  ...
  source_dir <- paste0("outputs/", spec_group, "/binary_envelope_tif")
  out_dir <- paste0("outputs/", spec_group, "/_pngs/binary_envelope_png")
  ...
  for(i in 1:length(n)){
    ...
    cols <- c("grey90", 'grey80', 'white','#006d2c')
    names(cols) <- labs
    names(dfr)[3] <- "class"
    
    dfr$category <- factor(dfr$class, levels = -1:2, labels = labs)
    ...
    temp_plot <- ggplot(dfr, aes(x, y, fill = category)) +
      ...
      scale_fill_manual(
        values = cols,
        name = "",
        breaks = labs[3:4]  # exclude "Land" from the legend
      ) +
