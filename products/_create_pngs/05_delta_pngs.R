...
for(j in 1:length(spec_group_all)){
  ...
  source_dir <- paste0("outputs/", spec_group, "/delta_tif")      # adjust as needed
  out_dir    <- paste0("outputs/", spec_group, "/_pngs/delta_png")
  ...
  for(i in 1:length(n)){
    ...
    temp_plot <- ggplot() +
      ...
      coord_equal() +
      scale_fill_distiller(name = "Relative probability\nabove presence\nthreshold",
